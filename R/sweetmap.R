#' Pretty correlation plots
#'
#' Implementation of pheatmap for correlation plots
#'
#' @import grid
#' @import RColorBrewer
#' @import scales
...
lo = function(rown, coln, nrow, ncol, cellheight = NA, cellwidth = NA, treeheight_col, treeheight_row, legend, annotation_row, annotation_col, annotation_colors, annotation_legend, annotation_names_row, annotation_names_col, main, fontsize, fontsize_row , fontsize_col, angle_col, gaps_row, gaps_col, ...){
  ...
  if(!is.null(coln[1]) | (!is.na2(annotation_row) & annotation_names_row)){
    ...
    # Set a fixed height for column names to avoid varying top space
  ...
  gt = gtable(widths = unit.c(rown_width, mat_width, treeheight_row, annot_row_width, legend_width, annot_legend_width),
              heights = unit.c(main_height, coln_height*0.707, annot_col_height, mat_height, treeheight_col),
...
draw_dendrogram = function(hc, gaps, horizontal = T){
  ...
  # This line flips it and makes it stay in place. It was all you needed to add
  x = unit(1, "npc") - x
...
draw_matrix = function(matrix, border_color, gaps_rows, gaps_cols, fmat, fontsize_number, number_color){
  ...
  # Remove lower triangular elements but keep the diagonal
  matrix[lower.tri(matrix, diag = FALSE)] <- NA
  fmat[lower.tri(fmat, diag = FALSE)] <- ''  # Replace display_numbers labels below the diagonal with ''

  # Draw the upper triangular part along with the diagonal
  res[["rect"]] = rectGrob(x = coord$x, y = coord$y, width = coord_x$size, height = coord_y$size, gp = gpar(fill = matrix, col = ifelse(is.na(matrix), NA, border_color)))
...
draw_colnames = function(coln, gaps, vjust_col, hjust_col, angle_col, ...){
  ...
  res = textGrob(coln, x = x, y = unit(1, "npc") + unit(3, "bigpts"), vjust = vjust_col, hjust = hjust_col, rot = 45, gp = gpar(...))
...
draw_rownames = function(rown, gaps, ...) {
  ...
  # Calculate the staggered x positions based on the diagonal alignment
  n = length(rown) # number of rows
  max_offset = 1 / n  # Adjust the maximum offset to bring the labels closer

  # Adjust x offsets for a more controlled staggering effect
  x_offsets = seq(0, by = max_offset, length.out = n)

  # Adjust x coordinates for labels to align with the diagonal, closer to the heatmap
  # Here we subtract from the initial x position to move labels left
  x = unit(-3, "bigpts") + unit(x_offsets, "npc")  # Adjust -3 to move more/less

  res = textGrob(rown, x = x, y = y, vjust = 0.5, hjust = 1, gp = gpar(...))
...
heatmap_motor = function(matrix, border_color, cellwidth, cellheight, tree_col, tree_row, treeheight_col, treeheight_row, filename, width, height, breaks, color, legend, annotation_row, annotation_col, annotation_colors, annotation_legend, annotation_names_row, annotation_names_col, main, fontsize, fontsize_row, fontsize_col, hjust_col, vjust_col, angle_col, fmat, fontsize_number, number_color, gaps_col, gaps_row, labels_row, labels_col, ...){
  ...
  # Adjust plot height and width if necessary
  if(!is.na(filename)){
    if(is.na(height)){
  ...
  if(!is.na(main)){
    ...
    res = gtable_add_grob(res, elem, t = 1, l = 2, name = "main", clip = "off")
  }

  if(length(labels_col) != 0){
    pars = list(labels_col, gaps = gaps_col, fontsize = fontsize_row, hjust_col = hjust_col, vjust_col = vjust_col, angle_col = angle_col, ...)  # Use fontsize_row for consistency
    elem = do.call(draw_colnames, pars)
    res = gtable_add_grob(res, elem, t = 4, l = 2, clip = "off", name = "col_names")
  }
  ...
  res = gtable_add_grob(res, elem, t = 4, l = 2, clip = "off", name = "matrix")

  # Draw rownames on the left
  if(length(labels_row) != 0){
    pars = list(labels_row, gaps = gaps_row, fontsize = fontsize_row, ...)
    elem = do.call(draw_rownames, pars)
    res = gtable_add_grob(res, elem, t = 4, l = 2, clip = "off", name = "row_names")
  }

  # Draw row dendrogram on the right
  if(!is.na2(tree_row)){
    elem = draw_dendrogram(tree_row, gaps_row, horizontal = FALSE)
    res = gtable_add_grob(res, elem, t = 4, l = 3, clip = "off", name = "row_tree")
  }
  ...
  if(!is.na2(annotation_col)){
    ...
    res = gtable_add_grob(res, elem, t = 3, l = 2, clip = "off", name = "col_annotation")

    # Draw names
    if(annotation_names_col){
      elem = draw_annotation_names(annotation_col, fontsize, horizontal = TRUE)
      res = gtable_add_grob(res, elem, t = 3, l = 3, clip = "off", name = "col_annotation_names")
    }
  }
  ...
  if(!is.na2(annotation_row)){
    ...
    res = gtable_add_grob(res, elem, t = 4, l = 4, clip = "off", name = "row_annotation")

    # Draw names
    if(annotation_names_row){
      elem = draw_annotation_names(annotation_row, fontsize, horizontal = FALSE, hjust_col = hjust_col, vjust_col = vjust_col, angle_col = angle_col)
      res = gtable_add_grob(res, elem, t = 5, l = 4, clip = "off", name = "row_annotation_names")
    }
  }
